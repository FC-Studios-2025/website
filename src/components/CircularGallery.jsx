...
// Create pagination dots component with click events
class PaginationDots {
  ...
  setActiveDot(index) {
    ...
    const actualIndex = index % this.itemCount;
...
class App {
  ...
  goToSlide(index) {
    ...
    // Reset auto-scroll timer to prevent immediate jump after manual navigation
    if (this.autoScroll) {
  ...
  createPaginationDots(items) {
    ...
    this.paginationDots = new PaginationDots({
      ...
      size: this.dotSize,
      gap: this.dotGap,
      onDotClick: (index) => this.goToSlide(index),
    });
  }
  ...
  update() {
    ...
    if (this.medias) {
      ...
      if (this.paginationDots && this.medias[0]) {
        const width = this.medias[0].width;
        const currentIndex =
          Math.round(Math.abs(this.scroll.current) / width) %
  ...
  addEventListeners() {
    ...
    // Keep touch/mouse events for dragging interaction
